...
Dans le cas où le magasin a coché la case _Envoyer les commandes à Magistor_ (`sync_orders`), une requête API à Shopify va récupérer toutes les commandes
- datant de **moins de 15J**
- ayant un **statut "non traitée"** (`unfulfilled`).

On vérifie pour chaque commande si elle n'a pas déjà été créée dans Ensapp et si ce n'est pas le cas, on va la créer.
...
- mode de livraison (`shipping_method`)* provenant soit :
  - d'un tag "transporteur@xxx"
  - du mode de livraison choisi
  - du mode de livraison par défaut
- magasin auquel elle est rattachée (`shop`) : ex. Aventure Bio

* Mode de livraison: 
le magasin peut cocher ou non _Faire le choix de la méthode d'expédition dans Magistor_ et remplir un ou plusieurs modes de livraison (avec nom, code transporteur et url de suivi).

La commande est maintenant créée avec un statut "initial" (`state`) dans Ensapp.
...
- avoir un vendeur transmissible si `vendor_to_fulfill` est renseigné => pour Aventure Bio = "Aventure Bio Grossiste"
